...
/* ─── NEW: BotSettings model for main menu toggle ───────── */
...
function isRateLimited(userId, limit = 20) {
  ...
  e.count++; rateLimitMap.set(userId, e);
...
function capLine(total, target) {
  ...
  const filled = Math.round(pct / 10), remain = Math.max(0, target - total);
  return '█'.repeat(filled) + '░'.repeat(10 - filled) + ' ' + pct + '%\n' +
    'የተመዘገበ: ' + total + ' ኪሎ | ቀሪ: ' + remain + ' ኪሎ | ኢላማ: ' + target + ' ኪሎ';
}

/* ─── 8. KEYBOARDS ──────────────────────────────────────── */
// mainKb: admin ዎች ሁሌ ያያሉ፤ ሌሎቹ setting ላይ ይወሰናል
async function mainKb(userId) {
  const isAdminUser = ADMIN_IDS.includes(userId);
  if (!isAdminUser) {
    const menuEnabled = await getSetting('mainMenuEnabled', true);
    if (!menuEnabled) return Markup.removeKeyboard();
  }
...
async function sendDocumentWithRetry(chatId, doc, extra, retries = 4) {
  ...
  for (let i = 0; i < retries; i++) {
    ...
    catch (e) { lastErr = e; if (i < retries - 1) await new Promise(r => setTimeout(r, 2500 * (i + 1))); }
  }
  throw lastErr;
}
...
async function handlePrint(ctx, routeId) {
  ...
  try {
    ...
    await sendDocumentWithRetry(ctx.chat.id, { source: buf, filename: fname }, {
...
function startDailyReportScheduler() {
  let last = '';
  setInterval(async () => {
    const eat = new Date(Date.now() + 3 * 60 * 60 * 1000), date = eat.toISOString().slice(0, 10);
...
bot.use(async (ctx, next) => {
  if (ctx.from?.id && !isAdmin(ctx) && isRateLimited(ctx.from.id)) return ctx.reply('ብዙ ጥያቄ — ትንሽ ይጠብቁ').catch(() => {});
  return next();
});
...
function welcomeText(name) {
  // FIX 4: prices pulled from GB_PRODUCTS (env-var driven), oil shows ብር/ሊትር
...
bot.start(async ctx => { ctx.session = {}; await ctx.reply(welcomeText(ctx.from?.first_name || 'እንኳን ደህና መጡ'), { parse_mode: "Markdown", ...await mainKb(ctx.from?.id) }); });
bot.command('help',  async ctx => { ctx.session = {}; await ctx.reply(welcomeText(ctx.from?.first_name || 'እንኳን ደህና መጡ'), { parse_mode: "Markdown", ...await mainKb(ctx.from?.id) }); });
...
bot.hears('📋 የምዝገባ ዝርዝሬ', async ctx => {
  ...
  if (!list.length) return ctx.reply('ምዝገባ የለዎትም። አቅጣጫ ይምረጡ', await mainKb(ctx.from?.id));
  for (const r of list) {
    const btns = [];
...
async function startRegistration(ctx, route) {
  ...
  await ctx.reply(`${route.emoji} *${route.label}*\n\nሙሉ ስምዎን ያስገቡ:`, { parse_mode: "Markdown", ...await mainKb(ctx.from?.id) });
}
...
bot.action(/^more_(.+)$/, async ctx => {
  await ctx.answerCbQuery().catch(() => {});
  const route = byRoute(ctx.match[1]); if (!route) return;
  ctx.session = { step: 'NAME', routeId: route.id, d: {} };
...
bot.action(/^pm_(.+)$/, async ctx => {
  ...
  // FIX 3: show service fee clearly + instruct to send screenshot
  await ctx.reply(
    `${m.emoji} *${m.label}*\n` +
    `━━━━━━━━━━━━━━━━\n` +
    `ቁጥር: \`${acct}\`\n\n` +
...
bot.on('text', async (ctx, next) => {
  ...
  if (step === 'GB_NAME') {
    ...
    ctx.session.gbName = txt; ctx.session.step = 'GB_PHONE';
  ...
  if (step === 'GB_PHONE') {
    ...
    ctx.session.gbPhone = phone; ctx.session.step = 'GB_KG';
  ...
  if (step === 'GB_KG') {
    ...
    if (!kg || kg <= 0 || kg > 5000) return ctx.reply('ትክክለኛ ቁጥር ያስገቡ (1–5000)');
    const prod = byProduct(ctx.session.gbProductId);
    const ul = unitLabel(prod);
    ...
    await GBReg.create({
      ...
      productId: gbProductId, fullName: gbName, phone: gbPhone,
      weightKg: kg, totalCost, pricePerKg: prod?.pricePerKg || 0,
    });
  ...
  if (step === 'NAME') {
    if (txt.length < 3) return ctx.reply('ሙሉ ስም ያስገቡ (3+ ፊደል)');
    ctx.session.d.name = txt; ctx.session.step = 'PHONE'; return ctx.reply('ስልክ ቁጥርዎን ያስገቡ:');
  }
  if (step === 'PHONE') {
    const phone = txt.replace(/\s/g, '');
    if (!/^0[79]\d{8}$/.test(phone) && !/^\+251[79]\d{8}$/.test(phone)) return ctx.reply('ትክክለኛ ስልክ ያስገቡ\nምሳሌ: 0912345678');
    ctx.session.d.phone = phone; ctx.session.step = 'CARGO'; return ctx.reply('ጭነት ዓይነት (ምን ዓይነት እቃ?):');
  }
  ...
  if (step === 'WEIGHT') {
    ...
    // FIX 3: show fee breakdown in summary before payment method selection
    return ctx.reply(
      `*ማጠቃለያ*\n━━━━━━━━━━━━━━━━\n` +
  ...
  if (step === 'SEND_NOTE') {
    ...
    if (!ready.length) return ctx.reply('ፈቃድ ያለው ምዝገባ የለም', await mainKb(ctx.from?.id));
    ...
    return ctx.reply(`ተልኳል — ${ready.length} ሰው (${sent} ደርሷቸዋል)`, await mainKb(ctx.from?.id));
  }
  return next();
});
...
bot.on('location', async (ctx, next) => {
  ...
  if (step === 'COL_LOC' && isAdmin(ctx)) {
    ...
    if (!list.length) return ctx.reply(`${ro?.label} — ምዝገባ የለም`, await mainKb(ctx.from?.id));
...
bot.hears('⏭️ ሳላጋራ ጨርስ', async ctx => {
  ...
  const regId = ctx.session.locRegId; ctx.session = {};
  await ctx.reply(`*ምዝገባ ተጠናቀቀ!*\n\nአድራሻ ኋላ ለማጨምር:\n"📋 የምዝገባ ዝርዝሬ" → "አድራሻ ላክ"\n\n${SUPPORT_PHONE}`, await mainKb(ctx.from?.id));
...
bot.hears('🔧 Admin', async ctx => {
  ...
  ctx.session = {};
  const menuEnabled = await getSetting('mainMenuEnabled', true);
  const toggleLabel = menuEnabled ? '🔴 Main Menu አጥፋ (OFF)' : '🟢 Main Menu አብራ (ON)';
  ...
  await ctx.reply('*የአስተዳዳሪ ፓነል*', { parse_mode: 'Markdown', ...Markup.inlineKeyboard([
    ...
    [Markup.button.callback(toggleLabel,                     'toggle_main_menu')],
  ]) });
});

/* ── NEW: Toggle main menu on/off ── */
bot.action('toggle_main_menu', async ctx => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  const current = await getSetting('mainMenuEnabled', true);
  const next = !current;
  await setSetting('mainMenuEnabled', next);
  const statusText = next ? '🟢 Main Menu ተከፈተ (ON)' : '🔴 Main Menu ተዘጋ (OFF)';
  await ctx.reply(`*${statusText}*\n\n${next ? 'ተጠቃሚዎች ቦቱን መጠቀም ይችላሉ።' : 'ተጠቃሚዎች ጊዜያዊ ዝግ መልዕክት ያያሉ።'}`, { parse_mode: 'Markdown' });
});
...
bot.action('gb_status', async ctx => {
  ...
  for (const prod of GB_PRODUCTS) {
    const ul = unitLabel(prod);
    const res = await GBReg.aggregate([{ $match: { productId: prod.id } }, { $group: { _id: null, kg: { $sum: '$weightKg' }, count: { $sum: 1 }, revenue: { $sum: '$totalCost' } } }]);
...
bot.action(/^lst_(.+)$/, async ctx => {
  ...
  for (const r of list) {
    const kb = r.status === 'reviewing' ? approveKb(r._id) : r.status === 'approved' ? Markup.inlineKeyboard([[Markup.button.callback('ሰርዝ', `no_${r._id}`)]]) : {};
    await ctx.reply(card(r, true), { parse_mode: 'Markdown', ...kb });
  }
});

async function setStatus(ctx, id, newStatus, notifyFn) {
  const r = await Reg.findByIdAndUpdate(id, { status: newStatus }, { new: true }); if (!r) return;
...
bot.action('admin_report', async ctx => {
  ...
  txt += '*አማራ ክልል → አዲስ አበባ*\n';
  for (const ro of ROUTES_TO_AA) { const counts = await Reg.aggregate([{ $match: { routeId: ro.id } }, { $group: { _id: '$status', n: { $sum: 1 } } }]); const m = {}; counts.forEach(c => { m[c._id] = c.n; }); const total = await routeWeight(ro.id); txt += `${ro.emoji} ${ro.label}\nፈቃድ: ${m.approved || 0} | ፍተሻ: ${m.reviewing || 0} | ያልከፈለ: ${m.pending || 0} | ተልኳል: ${m.sent || 0} | ${total}/${ro.targetKg} ኪሎ\n\n`; }
  await ctx.reply(txt, { parse_mode: 'Markdown' });
});

/* ── Collector ── */
bot.action('col_pick',       async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አቅጣጫ ምረጥ:', Markup.inlineKeyboard([[Markup.button.callback('አዲስ አበባ → አማራ ክልል', 'col_dir_toamhara')], [Markup.button.callback('አማራ ክልል → አዲስ አበባ', 'col_dir_toaa')]])); });
bot.action('col_dir_toamhara', async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አዲስ አበባ → አማራ ክልል:', Markup.inlineKeyboard(ROUTES_TO_AMHARA.map(r => [Markup.button.callback(`${r.emoji} ${r.label}`, `col_${r.id}`)]))); });
bot.action('col_dir_toaa',    async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አማራ ክልል → አዲስ አበባ:', Markup.inlineKeyboard(ROUTES_TO_AA.map(r => [Markup.button.callback(`${r.emoji} ${r.label}`, `col_${r.id}`)]))); });
bot.action(/^col_(.+)$/, async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); ctx.session = { step: 'COL_LOC', colRoute: ctx.match[1] }; await ctx.reply('ያሉበትን ቦታ ያጋሩ:', locKb()); });

/* ── Print ── */
bot.action('print_pick',       async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አቅጣጫ ምረጥ:', Markup.inlineKeyboard([[Markup.button.callback('አዲስ አበባ → አማራ ክልል', 'prnt_dir_toamhara')], [Markup.button.callback('አማራ ክልል → አዲስ አበባ', 'prnt_dir_toaa')]])); });
bot.action('prnt_dir_toamhara', async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አዲስ አበባ → አማራ ክልል — መስመር ምረጥ:', Markup.inlineKeyboard(ROUTES_TO_AMHARA.map(r => [Markup.button.callback(`${r.emoji} ${r.label}`, `prnt_${r.id}`)]))); });
bot.action('prnt_dir_toaa',    async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አማራ ክልል → አዲስ አበባ — መስመር ምረጥ:', Markup.inlineKeyboard(ROUTES_TO_AA.map(r => [Markup.button.callback(`${r.emoji} ${r.label}`, `prnt_${r.id}`)]))); });
bot.action(/^prnt_(.+)$/, async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await handlePrint(ctx, ctx.match[1]); });
...
bot.action('ch_dir_toamhara', async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አዲስ አበባ → አማራ ክልል:', Markup.inlineKeyboard(ROUTES_TO_AMHARA.map(r => [Markup.button.callback(`${r.emoji} ${r.label}`, `ch_ann_${r.id}`)]))); });
bot.action('ch_dir_toaa',    async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); await ctx.reply('አማራ ክልል → አዲስ አበባ:', Markup.inlineKeyboard(ROUTES_TO_AA.map(r => [Markup.button.callback(`${r.emoji} ${r.label}`, `ch_ann_${r.id}`)]))); });
bot.action('ch_test', async ctx => { if (!isAdmin(ctx)) { await ctx.answerCbQuery('ፈቃድ የለዎትም').catch(() => {}); return; } await ctx.answerCbQuery().catch(() => {}); if (!CHANNEL_ID) return ctx.reply('CHANNEL_ID አልተቀመጠም'); try { await bot.telegram.sendMessage(CHANNEL_ID, 'ፍተሻ ተሳክቷል'); await ctx.reply('ተሳክቷል'); } catch (e) { await ctx.reply(`አልተሳካም: ${e.message}`); } });
bot.action(/^ch_ann_(.+)$/, async ctx => {
  ...
  const ro = byRoute(ctx.match[1]); if (!ro) return;
  const total = await routeWeight(ro.id);
  try { await bot.telegram.sendMessage(CHANNEL_ID, `${ro.emoji} *${ro.label}*\n${capLine(total, ro.targetKg)}\n\nቀጥታ ከ ገበሬዎች — ርካሽ እና ፈጣን!\n${SUPPORT_PHONE}`, { parse_mode: 'Markdown' }); await ctx.reply(`ተልኳል — ${ro.label}`); }
  catch (e) { await ctx.reply(`አልተሳካም: ${e.message}`); }
});

/* ── Admin commands ── */
bot.command('report_now', async ctx => { if (!isAdmin(ctx)) return ctx.reply('ፈቃድ የለዎትም'); await sendDailyReport(); await ctx.reply('ሪፖርት ተልኳል'); });

bot.command('stats', async ctx => {
  if (!isAdmin(ctx)) return ctx.reply('ፈቃድ የለዎትም');
  const now = new Date(), date = now.toLocaleDateString('en-GB') + ' ' + now.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
  ...
  for (const ro of ROUTES_TO_AMHARA) {
    ...
    const m = {}; agg.forEach(c => { m[c._id] = { n: c.n, kg: c.kg }; });
    const people = agg.reduce((s, c) => s + c.n, 0), kg = ['pending','reviewing','approved','sent'].reduce((s, st) => s + (m[st]?.kg || 0), 0), rev = kg * SHIP_PER_KG;
...
bot.command('broadcast', async ctx => {
  ...
  let sent = 0, failed = 0;
  for (const uid of users) { try { await bot.telegram.sendMessage(uid, `${text}\n\n${SUPPORT_PHONE}`, { parse_mode: 'Markdown' }); sent++; } catch { failed++; } await new Promise(r => setTimeout(r, 50)); }
...
async function main() {
  ...
  await new Promise(resolve => { http.createServer((_, res) => { res.writeHead(200); res.end('OK'); }).listen(PORT, () => { console.log('Port', PORT); resolve(); }); });
  try { await bot.telegram.deleteWebhook({ drop_pending_updates: true }); console.log('Webhook deleted'); } catch (e) { console.warn('deleteWebhook:', e.message); }
  const RURL = (process.env.RENDER_EXTERNAL_URL || '').trim();
  if (RURL) {
    const https = require('https');
    setInterval(() => { try { const u = new URL(RURL); https.request({ hostname: u.hostname, path: '/', method: 'GET' }, r => console.log('keep-alive', r.statusCode)).on('error', () => {}).end(); } catch {} }, 9 * 60 * 1000);

...
function buildGBManifestHTML(prod, list) {
  ...
  for (const [nbr, regs] of nbrEntries) {
    ...
    const rows = regs.map((r) =>
      `<tr><td>${globalIdx++}</td><td>${r.fullName || "—"}</td><td>${r.phone || "—"}</td><td class="num">${r.weightKg || 0} ${ul}</td></tr>`
    ).join("");
    nbrDetailBlocks += `
<div class="nbr-block">
  <div class="nbr-header">
  ...
  <table>
    <thead><tr><th>#</th><th>ሙሉ ስም</th><th>ስልክ ቁጥር</th><th class="num">${ul}</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
</div>`;
  }
...
bot.action(/^prntgb_(.+)$/, async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  await handlePrintGB(ctx, ctx.match[1]);
});

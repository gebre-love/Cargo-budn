...
function houseDetailsLine(r) {
  ...
  if (r.sizeSqm)     parts.push(`📐 ${r.sizeSqm} m²`);
  return parts.join(" | ");
}
...
async function finalizeHouseListing(ctx) {
  const {
    houseDealType, houseName, housePhone, houseLocation, housePrice, houseDesc, houseMedia,
...
bot.on("text", async (ctx, next) => {
  ...
  if (step === "HOUSE_PRICE") {
    ...
    const dt = ctx.session.houseDealType;
    const descQ = HOUSE_SEEKING_TYPES.includes(dt)
      ? "📝 የሚፈልጉትን ቤት መረጃ ሁሉንም በአንድ ላይ ይጻፉ:\n(ዓይነት, የመኝታ ብዛት, የመታጠቢያ ብዛት, ወለል, ስፋት m², ወዘተ)"
      : "📝 ስለ ቤቱ መረጃ ሁሉንም በአንድ ላይ ይጻፉ:\n(ዓይነት, የመኝታ ብዛት, የመታጠቢያ ብዛት, ወለል, ስፋት m², ወዘተ)";
    return ctx.reply(descQ, { parse_mode: "Markdown", ...backKb() });
  }
  ...
  if (step === "HOUSE_DESC") {
    ...
    if (HOUSE_SEEKING_TYPES.includes(dt)) {
      /* ቤት ፈላጊዎች (ለመግዛት/ለመከራየት) — ፎቶ የለም፣ ቤት ስለሌላቸው — ወዲያው ይጠናቀቃል */
      ctx.session.houseMedia = [];
      await finalizeHouseListing(ctx);
      return;
    }

    /* ቤት ያላቸው (ለመሸጥ/ለማከራየት) — የግድ ፎቶ/ቪዲዮ ይጠየቃሉ */

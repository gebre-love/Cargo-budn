...
bot.hears("🏠 ቤት ኪራይ", async (ctx) => {
  ctx.session = { step: "HOUSE_NAME", houseDealType: "rent" };
  await ctx.reply(
    "🏠 *ቤት ለማከራየት/ለመከራየት ምዝገባ*\n━━━━━━━━━━━━━━━━\n\nሙሉ ስምዎን ያስገቡ:",
    { parse_mode: "Markdown", ...backKb() },
  );
});

bot.hears("🏘 ቤት ሽያጭ/ግዢ", async (ctx) => {
  ctx.session = { step: "HOUSE_NAME", houseDealType: "sale" };
  await ctx.reply(
    "🏘 *ቤት ለመሸጥ/ለመግዛት ምዝገባ*\n━━━━━━━━━━━━━━━━\n\nሙሉ ስምዎን ያስገቡ:",
    { parse_mode: "Markdown", ...backKb() },
  );
...
bot.hears("✅ ጨርሻለሁ", async (ctx) => {
  ...
  const { houseDealType, houseName, housePhone, houseLocation, housePrice, houseDesc, houseMedia } = ctx.session;
  ...
  const typeLabel = houseDealType === "rent" ? "🏠 ኪራይ" : "🏘 ሽያጭ/ግዢ";
  await ctx.reply(
    `✅ *ምዝገባዎ ተልኳል!*\n━━━━━━━━━━━━━━━━\n` +
  ...
  for (const aid of ADMIN_IDS) {
    bot.telegram.sendMessage(
      aid,
      `🆕 *አዲስ የቤት ምዝገባ* (${typeLabel})\n👤 ${houseName} | 📞 ${housePhone}\n📍 ${houseLocation}\n💰 ${housePrice}\n📝 ${houseDesc || "—"}\n🖼 ${houseMedia.length} ፎቶ/ቪዲዮ\nID: ${listing._id}`,
      { parse_mode: "Markdown" },
    ).catch(() => {});
    for (const m of houseMedia) {
...
bot.on("text", async (ctx, next) => {
  ...
  if (step === "HOUSE_PHONE") {
    ...
    return ctx.reply("📍 የቤቱ አድራሻ/አካባቢ ይጻፉ:", backKb());
  }

  if (step === "HOUSE_LOCATION") {
    ctx.session.houseLocation = txt.slice(0, 200);
    ctx.session.step = "HOUSE_PRICE";
    const label = ctx.session.houseDealType === "rent" ? "ወርሃዊ ኪራይ ዋጋ" : "የሽያጭ/ግዢ ዋጋ";
    return ctx.reply(`💰 ${label} ያስገቡ:`, backKb());
  }

  if (step === "HOUSE_PRICE") {
    ctx.session.housePrice = txt.slice(0, 60);
    ctx.session.step = "HOUSE_DESC";
    return ctx.reply("📝 ስለ ቤቱ አጭር መግለጫ ይጻፉ (ክፍሎች ብዛት, ወለል, ወዘተ):", backKb());
  }

  if (step === "HOUSE_DESC") {
    ctx.session.houseDesc = txt.slice(0, 500);

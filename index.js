...
/* ቻናል — Admin ካፀደቀ በኋላ የቤት ምዝገባ ራሱ የሚለጠፍበት (ለምሳሌ: @mychannel ወይም -100123456789) */
...
bot.action(/^house_appr_([a-f\d]{24})$/i, async (ctx) => {
  if (!isStaff(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  const listing = await HouseListing.findByIdAndUpdate(ctx.match[1], { status: "approved" }, { new: true }).lean();
  await ctx.editMessageReplyMarkup({ inline_keyboard: [] }).catch(() => {});
  await ctx.reply("✅ ጸድቋል");
  if (listing) await postListingToChannel(listing);
});

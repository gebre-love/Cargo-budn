...
// ── FIX 2: ቻናል ማስታወቂያ ውስጥም ቃሉ ተስተካክሏል ─────────────────
...
// ══ FIX 1: Print — ዝርዝር የሚያመጣ ══════════════════════════
bot.action('print_pick', async ctx => {
  if (!isAdmin(ctx)) return ctx.answerCbQuery('⛔').catch(() => {});
  await ctx.answerCbQuery().catch(() => {});
  await ctx.reply('🖨️ *የትኛው መስመር ዝርዝር ለፕሪንት?*', {
    parse_mode: 'Markdown',
    ...Markup.inlineKeyboard(
      ROUTES.map(r => [Markup.button.callback(`${r.emoji} ${r.label}`, `prnt_${r.id}`)])
    ),
  });
});

// ── FIX 1: "prnt_" prefix ተጠቀምን — "print_" ከ route id ጋር conflict አይፈጥርም ──
bot.action(/^prnt_([a-z_]+)$/, async ctx => {
  ...
  if (!ro) {
    console.error('prnt_: route not found for id:', routeId);
    return ctx.reply(`❗ መስመር አልተገኘም።`);
  }
  ...
  try {
    ...
    if (!list.length) {
      return ctx.reply(
        `${ro.emoji} *${ro.label}*\n\n📭 ምዝገባ የለም — ለዚህ መስመር ገና ምንም ደንበኛ አልተመዘገበም።`,
        { parse_mode: 'Markdown' }
      );
    }
...
async function main() {
  ...
  // HTTP health-check server (Render port scan)
  await new Promise(resolve => {
    http.createServer((_, res) => { res.writeHead(200); res.end('OK'); })
      .listen(PORT, () => { console.log('✅ Port', PORT); resolve(); });
  });

  // Webhook ሰርዝ — 409 Conflict እንዳይፈጠር
  try {
    await bot.telegram.deleteWebhook({ drop_pending_updates: true });
    console.log('✅ Webhook ተሰርዟል');
  } catch (e) {

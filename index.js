...
async function mainKb(userId) {
  ...
  rows.push(["🏠 ቤት ለመከራየት", "🔑 ቤት ለማከራየት"]);
  rows.push(["🏘 ቤት ለመግዛት", "💰 ቤት ለመሸጥ"]);
...
bot.on("text", async (ctx, next) => {
  ...
  if (step === "HOUSE_FLOOR") {
    const validOptions = ["Ground Floor", "1ኛ ፎቅ", "2ኛ ፎቅ", "3ኛ ፎቅ", "4+ ፎቅ"];
    ...
    if (isCondo(ctx.session.houseType)) {
      ctx.session.step = "HOUSE_KONDO_SIZE";
  ...
  if (step === "HOUSE_CASH_PRICE") {
    ...
    const dt = ctx.session.houseCashDealType;
    return ctx.reply(
      HOUSE_SEEKING_TYPES.includes(dt)
        ? "📝 የቤቱን አጠቃላይ መረጃ ያስገቡ:"
        : "📝 የቤቱን አጠቃላይ መረጃ ያስገቡ:",
      { parse_mode: "Markdown", ...backKb() },
    );
...
bot.action(/^prntgb_(?!nbr_)(.+)$/, async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  const prodId = ctx.match[1];
  const prod = byAnyProduct(prodId);
  ...
  await ctx.reply(
    `${prod.emoji} *${prod.label}* — ሰፈር ምረጥ:`,
    {
      ...
      ...Markup.inlineKeyboard([
        ...
        [Markup.button.callback("📋 ሁሉም ሰፈሮች", `prntgb_nbr_${prodId}____ALL__`)],

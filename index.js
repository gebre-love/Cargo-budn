...
function buildManifestHTML(ro, list, filterNbr = null) {
  ...
  const rows = sorted.map((r, i) => {
    const me = byMethod(r.paymentMethod);
    return `<tr><td>${i + 1}</td><td>${r.fullName || "—"}</td><td>${r.phone || "—"}${r.phoneUnverified ? " ⚠️" : ""}</td><td>${r.neighborhood || "—"}</td><td>${r.cargoDesc || "—"}</td><td class="num">${r.weightKg || 0}</td><td class="status status-${r.status}">${PRINT_STATUS[r.status] || r.status}</td></tr>`;
  }).join("");

  const css = `@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:'Noto Sans Ethiopic','Nyala',Arial,sans-serif;color:#1a1a1a;margin:0;padding:18px;font-size:12px}.letterhead{display:flex;justify-content:space-between;border-bottom:3px solid #1a3c6e;padding-bottom:10px;margin-bottom:12px}.letterhead h1{font-size:17px;margin:0 0 3px;color:#1a3c6e}.letterhead .meta{text-align:right;font-size:11px}.route-banner{background:#1a3c6e;color:#fff;padding:7px 14px;border-radius:4px;font-size:14px;font-weight:bold;margin-bottom:12px}.summary{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.box{border:1px solid #ccc;border-radius:5px;padding:6px 12px;text-align:center;background:#f7f8fa;min-width:90px}.box .v{font-size:17px;font-weight:bold;color:#1a3c6e}.box .l{font-size:10px;color:#666;margin-top:2px}table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:14px}th{background:#1a3c6e;color:#fff;padding:6px 5px;text-align:left}td{padding:5px;border-bottom:1px solid #ddd}tr:nth-child(even){background:#f5f6f8}.num{text-align:center}.status{text-align:center;font-weight:bold;font-size:10px}.status-approved{color:#1a7d3b}.status-sent{color:#1565c0}.status-reviewing{color:#b8860b}.status-pending{color:#888}.footer{margin-top:28px;display:flex;justify-content:space-between;font-size:11px}.sign-box{width:45%}.sign-line{border-top:1px solid #333;margin-top:34px;padding-top:4px;text-align:center}.stamp-note{margin-top:20px;font-size:10px;color:#777;text-align:center;border-top:1px dashed #ccc;padding-top:8px}#printBtn{margin:12px 0;padding:9px 26px;font-size:13px;background:#1a3c6e;color:#fff;border:none;border-radius:6px;cursor:pointer}@media print{#printBtn{display:none}body{padding:0}}`;
...
<table>
  <thead><tr><th>#</th><th>ሙሉ ስም</th><th>ስልክ</th><th>ሰፈር</th><th>ጭነት</th><th class="num">ኪሎ</th><th class="status">ሁኔታ</th></tr></thead>
  <tbody>${rows}</tbody>
...
function buildGBManifestHTML(prod, list, filterNbr) {
  ...
  const rows = list.map((r, i) =>
    `<tr><td>${i + 1}</td><td>${r.fullName || "—"}</td><td>${r.phone || "—"}</td><td class="num">${r.weightKg || 0} ${ul}</td></tr>`
  ).join("");

  const css = `@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:'Noto Sans Ethiopic','Nyala',Arial,sans-serif;color:#1a1a1a;margin:0;padding:18px;font-size:12px}.letterhead{display:flex;justify-content:space-between;border-bottom:3px solid #1a3c6e;padding-bottom:10px;margin-bottom:12px}.letterhead h1{font-size:17px;margin:0 0 3px;color:#1a3c6e}.letterhead .meta{text-align:right;font-size:11px}.route-banner{background:#1a3c6e;color:#fff;padding:7px 14px;border-radius:4px;font-size:14px;font-weight:bold;margin-bottom:12px}.summary{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.box{border:1px solid #ccc;border-radius:5px;padding:6px 12px;text-align:center;background:#f7f8fa;min-width:90px}.box .v{font-size:17px;font-weight:bold;color:#1a3c6e}.box .l{font-size:10px;color:#666;margin-top:2px}h3{color:#1a3c6e;font-size:13px;margin:14px 0 6px;border-bottom:1px solid #cdd6e0;padding-bottom:4px}table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:12px}th{background:#1a3c6e;color:#fff;padding:6px 5px;text-align:left}td{padding:5px;border-bottom:1px solid #ddd}tr:nth-child(even){background:#f5f6f8}.num{text-align:center}.footer{margin-top:28px;display:flex;justify-content:space-between;font-size:11px}.sign-box{width:45%}.sign-line{border-top:1px solid #333;margin-top:34px;padding-top:4px;text-align:center}#printBtn{margin:12px 0;padding:9px 26px;font-size:13px;background:#1a3c6e;color:#fff;border:none;border-radius:6px;cursor:pointer}@media print{#printBtn{display:none}body{padding:0}}`;
...
<table>
  ...
  <tbody>${rows}</tbody>
  <tfoot><tr style="font-weight:bold;background:#dde4ef"><td colspan="3">ጠቅላላ</td><td class="num">${totalKg} ${ul}</td></tr></tfoot>
</table>
...
bot.action("price_panel", async (ctx) => {
  ...
  buttons.push([Markup.button.callback(`📋 የምዝገባ ክፍያ — ${REG_PER_KG} ብር/ኪሎ`,  "adm_setfee_reg")]);
...
bot.action(/^pct_route_(.+)$/, async (ctx) => {
  ...
  let txt = `${ro.emoji} *${ro.label}* — በሰፈር % ድርሻ\n`;
  txt += `👥 ${list.length} ሰው | ${totalKg} ኪሎ\n`;
  txt += `${capLine(totalKg, ro.targetKg)}\n`;
  txt += breakdown.map((r) => `🏘 *${r.nbr}*: ${r.kg}ኪ — ${r.pct}%`).join("\n");
  await ctx.reply(txt, {
    parse_mode: "Markdown",
    ...Markup.inlineKeyboard([[Markup.button.callback("🔙 ተመለስ", "pct_pick")]]),
  });
});
...
bot.action(/^pct_gb_(.+)$/, async (ctx) => {
  ...
  if (!prod) return ctx.reply("ምርት አልተገኘም");
  const ul = unitLabel(prod);
  const list = await GBReg.find({ productId: prodId }).lean();
  if (!list.length) return ctx.reply(`${prod.emoji} ${prod.label} — ምዝገባ የለም`);
  const totalKg = list.reduce((s, r) => s + (r.weightKg || 0), 0);
  const breakdown = neighborhoodBreakdown(list);
  let txt = `${prod.emoji} *${prod.label}* — በሰፈር % ድርሻ\n`;
  txt += `👥 ${list.length} ሰው | ${totalKg} ${ul}\n`;
  txt += `${capLine(totalKg, prod.targetKg, ul)}\n`;
  txt += breakdown.map((r) => `🏘 *${r.nbr}*: ${r.kg}${ul} — ${r.pct}%`).join("\n");
  await ctx.reply(txt, {
    parse_mode: "Markdown",
    ...Markup.inlineKeyboard([[Markup.button.callback("🔙 ተመለስ", "pct_pick")]]),
  });

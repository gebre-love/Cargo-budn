...
/* ─── Dynamic admins (DB-stored, runtime-cached) ─────────── */
...
const METHODS = [
  ...
  { id: "cash",     emoji: "💵", label: "ጥሬ ገንዘብ (Cash)", info: "ጥሬ ገንዘብ"                                       },
];
...
/* ─── NEIGHBORHOOD LIST (ሰፈሮች) ─────────────────────────── */
const NEIGHBORHOODS = [
  "ቦሌ", "ኮልፌ ቀራኒዮ", "ጉለሌ", "ቃሊቲ", "አቃቂ ቃሊቲ",
  "ልደታ", "ቂርቆስ", "ይካ", "አዲስ ከተማ", "ሰሜን ምዕራብ",
  "አለም ባንክ", "ሜክሲኮ", "ፒያሳ", "ሉሲ", "ካዛንቺስ",
  "ጀሞ", "ሞሪ", "ጃናሞራ", "ሃና ማርያም", "ሰፈር ኦሮሞ",
  "ሌላ ሰፈር",
];
...
/* ─── [NEW] Dynamic admin helpers ────────────────────────── */
async function loadDynamicAdmins() {
  const saved = await getSetting("dynamic_admin_ids", []);
  dynamicAdminIds = new Set(Array.isArray(saved) ? saved.map(Number) : []);
}

async function addDynamicAdmin(userId) {
  dynamicAdminIds.add(Number(userId));
  await setSetting("dynamic_admin_ids", [...dynamicAdminIds]);
}

async function removeDynamicAdmin(userId) {
  dynamicAdminIds.delete(Number(userId));
  await setSetting("dynamic_admin_ids", [...dynamicAdminIds]);
}
...
/* ─── 7. HELPERS ────────────────────────────────────────── */
/* [NEW] isAdmin checks both static env list and dynamic DB admins */
const isAdmin = (ctx) =>
  ADMIN_IDS.includes(ctx.from?.id) || dynamicAdminIds.has(ctx.from?.id);

/* [NEW] isSuperAdmin — only the original static ADMIN_IDS can manage other admins */
const isSuperAdmin = (ctx) => ADMIN_IDS.includes(ctx.from?.id);
...
async function mainKb(userId) {
  const isAdminUser = ADMIN_IDS.includes(userId) || dynamicAdminIds.has(userId);
...
const dirRoutesKb = (routes) =>
  Markup.inlineKeyboard(routes.map((r) => [Markup.button.callback(`${r.emoji} ${r.label}`, `goto_${r.id}`)]));
...
async function checkCapacity(routeId) {
  ...
  if (total >= ro.targetKg && !cap.notified) {
    ...
    for (const aid of [...ADMIN_IDS, ...dynamicAdminIds])
...
/* ─── 11b. GB PRINT MANIFEST ────────────────────────────── */
function buildGBManifestHTML(prod, list) {
  const ul        = unitLabel(prod);
  const totalKg   = list.reduce((s, r) => s + (r.weightKg || 0), 0);
  const totalCost = list.reduce((s, r) => s + (r.totalCost || 0), 0);
  const totalFee  = list.reduce((s, r) => s + Math.round((r.weightKg || 0) * REG_PER_KG), 0);

  /* neighborhood breakdown */
  const nbrMap = {};
  for (const r of list) {
    const key = (r.neighborhood || "ያልተጠቀሰ").trim();
    if (!nbrMap[key]) nbrMap[key] = { kg: 0, count: 0 };
    nbrMap[key].kg    += r.weightKg || 0;
    nbrMap[key].count += 1;
  }
  ...
  const STATUS_AM = { pending: "ያልከፈለ", reviewing: "እየተፈተሸ", approved: "ፈቃድ ያለው" };
  const ORDER     = ["approved", "reviewing", "pending"];
  const sorted    = [...list].sort((a, b) => ORDER.indexOf(a.paymentStatus) - ORDER.indexOf(b.paymentStatus));

  const rows = sorted.map((r, i) => {
    const fee    = Math.round((r.weightKg || 0) * REG_PER_KG);
    const nbrKey = (r.neighborhood || "ያልተጠቀሰ").trim();
    const nbrKg  = nbrMap[nbrKey]?.kg || 0;
    const nbrCnt = nbrMap[nbrKey]?.count || 0;
    const nbrKgPct  = totalKg     > 0 ? Math.round((nbrKg  / totalKg)     * 100) : 0;
    const nbrCntPct = list.length > 0 ? Math.round((nbrCnt / list.length) * 100) : 0;
    return `<tr>
      <td>${i + 1}</td>
      <td>${r.fullName || "—"}</td>
      <td>${r.phone || "—"}${r.phoneUnverified ? " ⚠️" : ""}</td>
      <td>${r.neighborhood || "—"}</td>
      <td class="num">${r.weightKg || 0} ${ul}</td>
      <td class="num">${nbrKg} ${ul} (${nbrKgPct}%)</td>
      <td class="num">${nbrCnt} ሰው (${nbrCntPct}%)</td>
      <td class="num">${fee.toLocaleString("en")} ብር</td>
      <td class="status status-${r.paymentStatus}">${STATUS_AM[r.paymentStatus] || r.paymentStatus}</td>
    </tr>`;
  }).join("");

  /* neighborhood summary rows */
  const nbrRows = Object.entries(nbrMap)
    .sort((a, b) => b[1].kg - a[1].kg)
    .map(([name, d]) => {
      const ordPct = totalKg    > 0 ? Math.round((d.kg    / totalKg)    * 100) : 0;
      const regPct = list.length > 0 ? Math.round((d.count / list.length) * 100) : 0;
      return `<tr>
        <td><b>${name}</b></td>
        <td class="num">${d.count} ሰው — <b>${regPct}%</b></td>
        <td class="num">${d.kg} ${ul} — <b>${ordPct}%</b></td>
      </tr>`;
    }).join("");

  return `<!DOCTYPE html><html lang="am"><head><meta charset="UTF-8">
<title>${prod.emoji} ${prod.label} — GB ምዝገባ ዝርዝር</title>
<style>
@page{size:A4;margin:14mm}*{box-sizing:border-box}
body{font-family:'Noto Sans Ethiopic','Nyala',Arial,sans-serif;color:#1a1a1a;margin:0;padding:18px;font-size:13px}
.letterhead{display:flex;justify-content:space-between;border-bottom:3px solid #1a6e3c;padding-bottom:10px;margin-bottom:14px}
.letterhead h1{font-size:18px;margin:0 0 4px;color:#1a6e3c}
.letterhead .meta{text-align:right;font-size:12px}
.prod-banner{background:#1a6e3c;color:#fff;padding:8px 14px;border-radius:4px;font-size:15px;font-weight:bold;margin-bottom:14px}
...
.box .v{font-size:18px;font-weight:bold;color:#1a6e3c}.box .l{font-size:10px;color:#666;margin-top:2px}
h3{color:#1a6e3c;margin:14px 0 6px}
table{width:100%;border-collapse:collapse;font-size:12px;margin-bottom:18px}
th{background:#1a6e3c;color:#fff;padding:7px 6px;text-align:left}
td{padding:6px;border-bottom:1px solid #ddd}tr:nth-child(even){background:#f5f6f8}
.num{text-align:center}.status{text-align:center;font-weight:bold;font-size:11px}
.status-approved{color:#1a7d3b}.status-reviewing{color:#b8860b}.status-pending{color:#888}
.footer{margin-top:30px;display:flex;justify-content:space-between;font-size:12px}
.sign-box{width:45%}.sign-line{border-top:1px solid #333;margin-top:36px;padding-top:4px;text-align:center}
.stamp-note{margin-top:26px;font-size:11px;color:#777;text-align:center}
#printBtn{display:block;margin:16px auto;padding:14px 40px;font-size:16px;font-weight:bold;background:#1a6e3c;color:#fff;border:none;border-radius:8px;cursor:pointer;letter-spacing:1px}
#printHint{text-align:center;font-size:12px;color:#555;margin-bottom:10px}
@media print{#printBtn,#printHint{display:none}body{padding:0}}
</style></head><body>
<button id="printBtn" onclick="doPrint()">🖨 ፕሪንት ያድርጉ</button>
<div id="printHint">ፕሪንት dialog ካልተከፈተ ከላይ ያለውን button ይጫኑ</div>
<div class="letterhead">
  <div><h1>የቡድን ግዥ አገልግሎት</h1><div style="font-size:12px;color:#555">Group Buying Manifest</div></div>
  <div class="meta">${dateStr} &nbsp; ${timeStr}<br>${SUPPORT_PHONE}</div>
</div>
<div class="prod-banner">${prod.emoji} ${prod.label}</div>
<div class="summary">
  <div class="box"><div class="v">${list.length}</div><div class="l">ጠቅላላ ተሳታፊ</div></div>
  <div class="box"><div class="v">${totalKg}</div><div class="l">ጠቅላላ ${ul}</div></div>
  <div class="box"><div class="v">${prod.targetKg}</div><div class="l">ኢላማ ${ul}</div></div>
  <div class="box"><div class="v">${totalFee.toLocaleString("en")}</div><div class="l">አ. ክፍያ (ብር)</div></div>
  <div class="box"><div class="v">${totalCost.toLocaleString("en")}</div><div class="l">ጠቅላላ ዋጋ (ብር)</div></div>
  <div class="box"><div class="v">${prod.pricePerKg}</div><div class="l">ዋጋ/${ul} (ብር)</div></div>
</div>

<h3>📍 በሰፈር ዝርዝር</h3>
<table><thead><tr><th>ሰፈር</th><th class="num">ተሳታፊ (%)</th><th class="num">ምርት (%)</th></tr></thead>
<tbody>${nbrRows}</tbody></table>

<h3>📋 ሙሉ ዝርዝር</h3>
<table><thead>
  <tr><th>#</th><th>ሙሉ ስም</th><th>ስልክ ቁጥር</th><th>ሰፈር</th><th class="num">${ul}</th><th class="num">ሰፈር ምርት</th><th class="num">ሰፈር ሰዎች</th><th class="num">አ. ክፍያ</th><th class="status">ሁኔታ</th></tr>
</thead><tbody>${rows}</tbody></table>

<div class="footer">
  <div class="sign-box"><div class="sign-line">ስም እና ፊርማ — Name &amp; Signature</div></div>
  <div class="sign-box"><div class="sign-line">ተቀባይ ባለሥልጣን — Receiving Officer</div></div>
</div>
<div class="stamp-note">ይህ ሰነድ ለ${prod.label} የቡድን ግዥ ምዝገባ ዝርዝር ነው — ${dateStr}</div>
<script>
function doPrint(){try{window.print();}catch(e){alert('ፕሪንት ለማድረግ browser ይጠቀሙ');}}
window.addEventListener('load',function(){setTimeout(function(){try{window.print();}catch(e){}},600);});
</script>
</body></html>`;
}

async function handleGBPrint(ctx, productId) {
  const prod = byProduct(productId);
  if (!prod) { await ctx.reply("ምርት አልተገኘም"); return; }
  let waitMsg;
  try {
    waitMsg = await ctx.reply("GB ሰነዱ እየተዘጋጀ ነው፣ ትንሽ ይጠብቁ...");
    ...
    const totalKg = list.reduce((s, r) => s + (r.weightKg || 0), 0);
    const html    = buildGBManifestHTML(prod, list);
    const buf     = Buffer.from(html, "utf-8");
    const fname   = `GB_${prod.id}_${new Date().toISOString().slice(0, 10)}.html`;
    await sendDocumentWithRetry(
      ctx.chat.id,
      { source: buf, filename: fname },
      {
        caption:    `*${prod.emoji} ${prod.label}* — GB ፕሪንት ዝግጁ\n${list.length} ሰው | ${totalKg} ${unitLabel(prod)}\n\nፋይሉን ይክፈቱ — ፕሪንት ይከፈታል`,
        parse_mode: "Markdown",
      },
    );
    if (waitMsg) bot.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id).catch(() => {});
  } catch (e) {
    console.error("handleGBPrint:", e.message);
    await ctx.reply("GB ፋይሉን መላክ አልተሳካም — ትንሽ ቆይተው ይሞክሩ።").catch(() => {});
  }
}
...
/* ─── [NEW] DAILY CASH COLLECTION REMINDER ──────────────── */
function startCashReminderScheduler() {
  let lastReminder = "";
  setInterval(async () => {
    try {
      // Run every day at 09:00 EAT (UTC+3)
      const eat  = new Date(Date.now() + 3 * 60 * 60 * 1000);
      const date = eat.toISOString().slice(0, 10);
      if (eat.getUTCHours() === 6 && eat.getUTCMinutes() === 0 && lastReminder !== date) {
        lastReminder = date;

        // Find all pending cash cargo registrations
        const pending = await Reg.find({ paymentMethod: "cash", status: "reviewing" }).lean();

        if (pending.length === 0) return; // nothing to remind about

        const lines = pending.map((r, i) => {
          const ro   = byRoute(r.routeId);
          const when = new Date(r.createdAt).toLocaleDateString("en-GB");
          return `${i + 1}. *${r.fullName || "—"}* | ${r.phone || "—"} | ${ro?.label || r.routeId} | ${r.weightKg} ኪሎ | ${r.totalPrice} ብር | 📍 ${r.neighborhood || "—"} | ${when}`;
        }).join("\n");

        const msg =
          `⏰ *የጥሬ ገንዘብ ክፍያ ማስታወሻ — ${date}*\n\n` +
          `💵 *${pending.length}* ደንበኛ ጥሬ ገንዘብ ክፍያ ይጠብቃሉ:\n\n` +
          lines +
          `\n\n✅ ክፍያ ከተሰበሰበ /admin ሄደው ያረጋግጡ።`;

        // Notify all admins (super + dynamic)
        const allAdmins = new Set([...ADMIN_IDS, ...dynamicAdminIds]);
        for (const aid of allAdmins) {
          bot.telegram.sendMessage(aid, msg, { parse_mode: "Markdown" }).catch(() => {});
        }
      }
    } catch (e) {
      console.error("Cash reminder error:", e.message);
    }
  }, 60_000);
}

/* ─── [NEW] BACKUP HELPER ───────────────────────────────── */
async function doBackup(adminId, replyFn) {
  await replyFn("⏳ Backup እየተዘጋጀ ነው — ትንሽ ይጠብቁ...");
...
function defaultWelcomeText(name) {
  return (
    ...
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `📋 *የምዝገባ ክፍያ:* ${REG_PER_KG} ብር/ኪሎ\n` +
    `_የምርት ዋጋ በ ቻናላችን እናሳውቃለን_\n\n` +
    `📞 ለጥያቄ: ${SUPPORT_PHONE}\n\n` +
    `*ከታች የሚፈልጉትን ምርት ይጫኑ:*`
  );
}
...
bot.hears("📋 የምዝገባ ዝርዝሬ", async (ctx) => {
  ...
  for (const g of gbRegs) {
    ...
    await ctx.reply(
      ...
      `👥 ተሳታፊ: ${regCount} ሰው`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("➕ ኪሎ ጨምር", `gb_addkg_${g._id}`)]]) },
    );
  }
});
...
bot.action(/^nbr_(.+)$/, async (ctx) => {
  ...
  if (step === "NEIGHBORHOOD") {
    ctx.session.d.neighborhood = nbr;
    ctx.session.step = "PHONE";
    return ctx.reply(`🏘 *${nbr}* — ምርጫዎ ተቀብሏል\n\nስልክ ቁጥርዎን ያስገቡ (ምሳሌ: 0912345678):`, { parse_mode: "Markdown" });
  }
  if (step === "GB_NEIGHBORHOOD") {
    ctx.session.gbNeighborhood = nbr;
    ctx.session.step = "GB_PHONE";
    return ctx.reply(`🏘 *${nbr}*\n\nስልክ ቁጥርዎን ያስገቡ (ምሳሌ: 0912345678):`, { parse_mode: "Markdown" });
  }
});
...
bot.action(/^pm_(.+)$/, async (ctx) => {
  ...
  if (m.id === "cash") {
    ...
    await ctx.reply(
      ...
      `🚶 ሰራተኛ ቤትዎ ጥሬ ገንዘቡን ይሰበሰቡዎታሎ — ዝጉ ዝጁ ይሁኑ!\n📞 ${SUPPORT_PHONE}`,
      { parse_mode: "Markdown" },
    );
    /* ask for location */
    ctx.session = { step: "LOC", locRegId: String(r._id) };
    await ctx.reply("📍 ሰራተኛ ፈጥኖ እንዲደርስ አድራሻዎን ያጋሩ:", locKb());
    /* notify all admins */
    const ro = byRoute(routeId);
    const cashKb = Markup.inlineKeyboard([[
      Markup.button.callback("✅ ጥሬ ተሰብስቧል",  `ok_${r._id}`),
    ...
    for (const aid of [...ADMIN_IDS, ...dynamicAdminIds])
      bot.telegram.sendMessage(
        aid,
        `💵 *ጥሬ ገንዘብ ምዝገባ (Cash)*\n━━━━━━━━━━━━━━━━\n\n` +
        `ስም: *${d.name}*\nስልክ: \`${d.phone}\`${d.phoneUnverified ? " ⚠️" : ""}\n` +
        `ሰፈር: ${d.neighborhood || "—"}\n` +
        `አቅጣጫ: ${ro?.label || routeId}\n` +
        `ጭነት: ${d.cargo} — *${d.kg} ኪሎ*\n` +
        `💳 ክፍያ: *${r.totalPrice} ብር*\n\n` +
        `⬇️ ጥሬ ገንዘቡን ቤቱ ሄደው ሲሰበስቡ "✅ ጥሬ ተሰብስቧል" ይጫኑ`,
        { parse_mode: "Markdown", ...cashKb },
      ).catch(() => {});
    if (d.phoneUnverified)
      for (const aid of ADMIN_IDS)
        bot.telegram.sendMessage(
          aid,
          `⚠️ *ስልክ ማረጋገጫ ያስፈልጋል (Cash)*\n\nስም: ${d.name}\nስልክ: \`${d.phone}\``,
...
bot.on("text", async (ctx, next) => {
  ...
  /* [NEW] Make-admin: waiting for a user ID to promote */
  if (step === "ADMIN_MAKE_ADMIN_ID") {
    if (!isSuperAdmin(ctx)) { ctx.session = {}; return ctx.reply("ፈቃድ የለዎትም"); }
    const targetId = Number(txt.replace(/\D/g, ""));
    if (!targetId) return ctx.reply("❌ ትክክለኛ Telegram ID ያስገቡ (ቁጥር ብቻ):");
    if (ADMIN_IDS.includes(targetId)) { ctx.session = {}; return ctx.reply("✅ ይህ ሰው አስቀድሞ Super Admin ነው።"); }
    if (dynamicAdminIds.has(targetId)) { ctx.session = {}; return ctx.reply(`✅ ይህ ሰው (${targetId}) አስቀድሞ Admin ነው።`); }
    ctx.session = { step: "ADMIN_MAKE_ADMIN_CONFIRM", pendingAdminId: targetId };
    return ctx.reply(
      `👤 *Admin ለማድረግ:* \`${targetId}\`\n\nይህ ሰው የBot Admin ሆኖ ሁሉንም admin ፓነሎች ማየት ይችላሉ።\n\nእርግጠኛ ነዎት?`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[
        Markup.button.callback("✅ አዎ ፍቀድ", "confirm_make_admin"),
        Markup.button.callback("❌ ሰርዝ",    "cancel_make_admin"),
      ]]) },
    );
  }
  ...
  if (step === "GB_NAME") {
    ...
    return ctx.reply(
      `👤 ${txt}\n\nሰፈርዎ የት ነው? (ከዝርዝሩ ይምረጡ):`,
      { parse_mode: "Markdown", ...neighborhoodKb() },
    );
  }

  if (step === "GB_NEIGHBORHOOD") {
    if (txt.length < 2) return ctx.reply("ሰፈርዎን ይምረጡ ወይም ይጻፉ:");
  ...
  if (step === "GB_KG") {
    ...
    return ctx.reply(
      ...
      `💳 *የምዝገባ (አገልግሎት) ክፍያ: ${serviceFee} ብር*\n` +
      `_(${kg} ${ul} × ${REG_PER_KG} ብር/${ul})_\n\n` +
      `${capLine(regKg, prod.targetKg, ul)}\n` +
      `👥 ተሳታፊ: ${regCount} ሰው\n\n` +
      `ክፍያ ዘዴ ይምረጡ:`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([
        [Markup.button.callback("✅ ቀጥል", "gb_confirm_yes")],
        [Markup.button.callback("❌ ሰርዝ",  "gb_confirm_no")],
      ]) },
    );
  }
  ...
  if (step === "GB_ADDKG") {
    ...
    if (!newKg || newKg <= 0 || newKg > 5000) return ctx.reply("ትክክለኛ ቁጥር ያስገቡ (1–5000)");
    const { gbAddOldKg, gbAddId, gbAddProductId } = ctx.session;
    const prod = byProduct(gbAddProductId), ul = unitLabel(prod);
    if (newKg <= gbAddOldKg) return ctx.reply(`አሁናዊ: ${gbAddOldKg} ${ul}\nአዲሱ ቁጥር ከዚህ በላይ መሆን አለበት:`);
    const diff    = Math.round((newKg - gbAddOldKg) * 10) / 10;
    const diffFee = Math.round(diff * REG_PER_KG);
    ctx.session.gbAddNewKg  = newKg;
    ctx.session.gbAddDiffKg = diff;
    ctx.session.gbAddDiffFee = diffFee;
    ctx.session.step = "GB_ADDKG_CONFIRM";
    return ctx.reply(
      `*ኪሎ ማሻሻያ*\n━━━━━━━━━━━━━━━━\n` +
      `${prod?.emoji} *${prod?.label}*\nቀድሞ: ${gbAddOldKg} ${ul} → አዲስ: *${newKg} ${ul}*\n` +
      `ተጨማሪ: ${diff} ${ul}\n\n` +
      `💳 ተጨማሪ ክፍያ: *${diffFee} ብር*\n\nይቀጥሉ?`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[
        Markup.button.callback("✅ ቀጥል",   "gb_addkg_confirm"),
        Markup.button.callback("❌ ሰርዝ",    "gb_addkg_cancel"),
      ]]) },
    );
  }
  ...
  if (step === "NAME") {
    ...
    return ctx.reply(
      `👤 ${txt}\n\nሰፈርዎ የት ነው? (ከዝርዝሩ ይምረጡ ወይም ይጻፉ):`,
      { parse_mode: "Markdown", ...neighborhoodKb() },
    );
  }

  if (step === "NEIGHBORHOOD") {
    if (txt.length < 2) return ctx.reply("ሰፈርዎን ይምረጡ ወይም ይጻፉ (ቢያንስ 2 ፊደል):");
  ...
  if (step === "PHONE") {
    ...
    if (!phoneValid) {
      ...
      await ctx.reply(`⚠️ ስልክ ቁጥሩ ቅርጸቱ ልዩ ነው — ሰራተኛ ያረጋግጣሉ.\n\nጭነት ዓይነት (ምን ዓይነት እቃ?):`);
      return;
    }
    ctx.session.d.phone = phone;
    ctx.session.step    = "CARGO";
  ...
  if (step === "CARGO") {
    if (txt.length < 2 || txt.length > 200) return ctx.reply("ጭነቱን ያስገቡ (2–200 ፊደል):");
...
bot.on("photo", async (ctx) => {
  ...
  if (step === "GB_AWAIT_PHOTO" && gbProductId) {
    ...
    await ctx.reply(
      autoOk
        ? `✅ *ክፍያ ተፈቅዷል — ምዝገባ ተጠናቀቀ!*\n━━━━━━━━━━━━━━━━\n` +
          `${prod?.emoji} *${prod?.label}* — ${gbKg} ${ul}\n` +
          `👤 ${gbName}  |  📞 ${gbPhone}\n🏘 ሰፈር: ${gbNeighborhood || "—"}\n\n` +
          `${capLine(regKg, prod?.targetKg || 5000, ul)}\n` +
          `👥 ተሳታፊ: ${regCount} ሰው\n\n` +
          `✨ _ምዝገባ ሲሞላ እናሳውቅዎታለን!_\n📞 ${SUPPORT_PHONE}`
        : `⏳ ፎቶ ደርሷል። ክፍያ ሰራተኛ ያረጋግጣሉ — ትንሽ ይጠብቁ.\n${SUPPORT_PHONE}`,
      { parse_mode: "Markdown", ...(await mainKb(ctx.from?.id)) },
    );
...
function adminPanelKb(grpOn) {
  ...
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback("🖨 GB ዝርዝር አትም",                 "gb_print_pick")],
    ...
    /* [NEW] Backup and Make-Admin buttons */
    [Markup.button.callback("💾 Backup",                        "admin_backup"),
     Markup.button.callback("👤 Admin ማድረጊያ",                  "make_admin_panel")],
  ]);
}
...
bot.action("admin_backup", async (ctx) => {
  ...
  await ctx.answerCbQuery("💾 Backup እየተዘጋጀ ነው...").catch(() => {});
  const adminId = ctx.from.id;
  await doBackup(adminId, (msg, opts) => ctx.reply(msg, opts));
});

/* ─── [NEW] MAKE ADMIN PANEL ────────────────────────────── */
bot.action("make_admin_panel", async (ctx) => {
  if (!isSuperAdmin(ctx)) {
    await ctx.answerCbQuery("ይህ ለ Super Admin ብቻ ነው").catch(() => {});
    return;
  }
  await ctx.answerCbQuery().catch(() => {});

  const dynList = [...dynamicAdminIds];
  let txt = `*👤 Admin ማስተዳዳሪ*\n━━━━━━━━━━━━━━━━\n\n`;
  if (dynList.length) {
    txt += `*አሁናዊ Dynamic Admins:*\n`;
    dynList.forEach((id, i) => { txt += `${i + 1}. \`${id}\`\n`; });
    txt += "\n";
  } else {
    txt += `_ምንም dynamic admin የለም_\n\n`;
  }
  txt += `ለጤፍ/የሰፈር ሃሳፊዎች ብቻ admin ለማድረግ Telegram User ID ያስፈልጋል።\n\nአዲስ ሰው ለማከል "➕ Admin ጨምር" ይጫኑ።\nAdmin ለማስወገድ "➖ Admin አስወጣ" ይጫኑ።`;

  const buttons = [];
  buttons.push([Markup.button.callback("➕ Admin ጨምር", "make_admin_add")]);
  if (dynList.length) {
    dynList.forEach((id) => {
      buttons.push([Markup.button.callback(`➖ አስወጣ: ${id}`, `remove_admin_${id}`)]);
    });
  }
  buttons.push([Markup.button.callback("↩️ ተመለስ", "back_to_admin")]);
  await ctx.reply(txt, { parse_mode: "Markdown", ...Markup.inlineKeyboard(buttons) });
});

bot.action("make_admin_add", async (ctx) => {
  if (!isSuperAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  ctx.session = { step: "ADMIN_MAKE_ADMIN_ID" };
  await ctx.reply(
    `*👤 አዲስ Admin ጨምር*\n━━━━━━━━━━━━━━━━\n\n` +
    `Admin ሊሆን የሚፈልጉት ሰው *Telegram User ID* ያስገቡ\n\n` +
    `_ሰዉ /start ቢልኩ — ID ማግኘት ይቻላል, ወይም @userinfobot ይጠቀሙ_`,
    { parse_mode: "Markdown" },
  );
});

bot.action("confirm_make_admin", async (ctx) => {
  if (!isSuperAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery("✅ Admin ተጨምሯል").catch(() => {});
  const { pendingAdminId } = ctx.session || {};
  if (!pendingAdminId) return ctx.reply("❌ ስህተት — ዳግም ይሞክሩ");
  await addDynamicAdmin(pendingAdminId);
  ctx.session = {};
  await ctx.editMessageReplyMarkup({ inline_keyboard: [] }).catch(() => {});
  await ctx.reply(
    `✅ *Admin ተጨምሯል!*\n\`${pendingAdminId}\` አሁን Admin ናቸው — Bot ሁሉንም አስተዳዳሪ ፓነሎች ማየት ይችላሉ።`,
    { parse_mode: "Markdown" },
  );
  bot.telegram.sendMessage(pendingAdminId,
    `✅ *Admin ሆነዋል!*\n\nNow you have admin access to this bot.\nUse "🔧 Admin" to access the admin panel.`,
    { parse_mode: "Markdown" },
  ).catch(() => {});
});

bot.action("cancel_make_admin", async (ctx) => {
  await ctx.answerCbQuery("ተሰርዟል").catch(() => {});
  ctx.session = {};
  await ctx.editMessageReplyMarkup({ inline_keyboard: [] }).catch(() => {});
  await ctx.reply("Admin ማድረጊያ ተሰርዟል.");
});

bot.action(/^remove_admin_(\d+)$/, async (ctx) => {
  if (!isSuperAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery("❌ Admin ተወግዷል").catch(() => {});
  const targetId = Number(ctx.match[1]);
  await removeDynamicAdmin(targetId);
  await ctx.reply(
    `❌ *Admin ተወግዷል*\n\`${targetId}\` አሁን Admin አይደሉም።`,
    { parse_mode: "Markdown" },
  );
  bot.telegram.sendMessage(targetId,
    `❌ Admin access has been removed from your account.`,
  ).catch(() => {});
});
...
bot.action("gb_addkg_cancel", async (ctx) => {
  await ctx.answerCbQuery().catch(() => {});
  ctx.session = {};
  await ctx.reply("ኪሎ ማሻሻያ ተሰርዟል.", await mainKb(ctx.from?.id));
});
...
bot.action("gb_status", async (ctx) => {
  ...
  for (const prod of GB_PRODUCTS) {
    ...
    const totalKg = res[0]?.kg || 0, totalCount = res[0]?.count || 0;
    ...
    let txt = `${prod.emoji} *${prod.label}*\n`;
    txt += `━━━━━━━━━━━━━━━━\n`;
    txt += `👥 *${totalCount} ሰው | ${totalKg} ${ul}*\n`;
    txt += `🎯 ኢላማ: ${prod.targetKg} ${ul}\n\n`;

    if (nbrAgg.length > 0) {
      txt += `*📍 በሰፈር ዝርዝር:*\n\n`;
      for (const row of nbrAgg) {
        const nbrName    = (row._id || "ያልተጠቀሰ").toString().trim();
        const orderedPct = totalKg    > 0 ? Math.round((row.kg    / totalKg)    * 100) : 0;
        const regPct     = totalCount > 0 ? Math.round((row.count / totalCount) * 100) : 0;
        txt += `🏘 *${nbrName}*\n`;
        txt += `📦 ምርት: ${row.kg} ${ul} — ${orderedPct}%\n`;
        txt += `👤 ተሳታፊ: ${row.count} ሰው — ${regPct}%\n\n`;
      }
    } else {
      txt += `_ምዝገባ ገና ያልተጀመረ_\n`;
    }
...
/* ── GB Print Pick ───────────────────────────────────────── */
bot.action("gb_print_pick", async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  await ctx.reply(
    "*🖨 GB ዝርዝር አትም*\nምርት ምረጡ:",
    Markup.inlineKeyboard([
      ...GB_PRODUCTS.map((p) => [Markup.button.callback(`${p.emoji} ${p.label}`, `gb_print_${p.id}`)]),
      [Markup.button.callback("↩️ ተመለስ", "back_to_admin")],
    ]),
  );
});

bot.action(/^gb_print_(.+)$/, async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery("🖨 GB ፕሪንት እየተዘጋጀ ነው...").catch(() => {});
  const productId = ctx.match[1];
  await handleGBPrint(ctx, productId);
});
...
bot.action("lst_dir_toamhara", async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  await ctx.reply("አዲስ አበባ → አማራ ክልል — መስመር ምረጥ:", Markup.inlineKeyboard(ROUTES_TO_AMHARA.map((r) => [Markup.button.callback(`${r.emoji} ${r.label}`, `lst_${r.id}`)])));
});
bot.action("lst_dir_toaa", async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  await ctx.reply("አማራ ክልል → አዲስ አበባ — መስመር ምረጥ:", Markup.inlineKeyboard(ROUTES_TO_AA.map((r) => [Markup.button.callback(`${r.emoji} ${r.label}`, `lst_${r.id}`)])));
});
...
bot.action("prnt_dir_toamhara", async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  await ctx.reply("አዲስ አበባ → አማራ ክልል:", Markup.inlineKeyboard(ROUTES_TO_AMHARA.map((r) => [Markup.button.callback(`${r.emoji} ${r.label}`, `prnt_${r.id}`)])));
});
bot.action("prnt_dir_toaa", async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  await ctx.reply("አማራ ክልል → አዲስ አበባ:", Markup.inlineKeyboard(ROUTES_TO_AA.map((r) => [Markup.button.callback(`${r.emoji} ${r.label}`, `prnt_${r.id}`)])));
});
...
/* [CHANGE 3] Channel announcement — per-neighborhood breakdown, NO overall % */
bot.action(/^ch_ann_(.+)$/, async (ctx) => {
  ...
  /* Query all active registrations grouped by neighborhood */
  const regs = await Reg.find({ routeId: ro.id, status: { $in: ACTIVE } })
    .sort({ neighborhood: 1, createdAt: 1 }).lean();
  ...
  const grouped = {};
  for (const r of regs) {
    const key = (r.neighborhood || "ሰፈር ያልተጠቀሰ").trim();
    if (!grouped[key]) grouped[key] = { kg: 0, count: 0 };
    grouped[key].kg    += r.weightKg || 0;
    grouped[key].count += 1;
  }

  /* Build message — neighborhood breakdown only, no overall % */
  let msg = `${ro.emoji} *${ro.label}*\n`;
  msg += `━━━━━━━━━━━━━━━━\n`;
  msg += `👥 *${totalPeople} ሰው | ${totalKg} ኪሎ*\n`;
  msg += `🎯 ኢላማ: ${ro.targetKg} ኪሎ\n`;
  msg += `━━━━━━━━━━━━━━━━\n\n`;
  msg += `*📍 በሰፈር ዝርዝር:*\n\n`;

  const sorted = Object.entries(grouped).sort((a, b) => b[1].kg - a[1].kg);
  for (const [nbr, data] of sorted) {
    const bar = Math.round((data.kg / ro.targetKg) * 10);
    msg += `🏘 *${nbr}*\n`;
    msg += `${"█".repeat(bar)}${"░".repeat(Math.max(0, 10 - bar))}\n`;
    msg += `${data.count} ሰው | ${data.kg} ኪሎ\n\n`;
  }

  if (!totalPeople) {
    msg += `_ምዝገባ ገና ያልተጀመረ_\n\n`;
  }
...
export async function startBot() {
  ...
  await loadDynamicAdmins(); /* [NEW] load dynamic admins from DB */
  console.log("Prices and admin list loaded from DB");

...
function buildManifestHTML(ro, list, filterNbr = null) {
  ...
  const nbrMap = {};
  for (const r of list) {
    const key = (r.neighborhood || "ሌላ").trim();
    if (!nbrMap[key]) nbrMap[key] = [];
    nbrMap[key].push(r);
  }
  const nbrEntries = Object.entries(nbrMap).sort((a, b) => a[0].localeCompare(b[0]));

  /* ── የሰፈር ማጠቃለያ ረድፎች (summary table) ── */
  const nbrSummaryRows = nbrEntries.map(([nbr, regs]) => {
    const kg  = regs.reduce((s, r) => s + (r.weightKg || 0), 0);
    const pct = totalKg > 0 ? Math.round((kg / totalKg) * 100) : 0;
    return `<tr><td>${nbr}</td><td class="num">${regs.length}</td><td class="num">${kg}</td><td class="num">${pct}%</td></tr>`;
  }).join("") +
  `<tr style="font-weight:bold;background:#dde4ef"><td>ጠቅላላ</td><td class="num">${list.length}</td><td class="num">${totalKg}</td><td class="num">100%</td></tr>`;

  /* ── የሰፈር ዝርዝር ሠንጠረዦች (per-neighborhood tables) ── */
  const ORDER = ["approved", "sent", "reviewing", "pending"];
  let nbrDetailBlocks = "";
  let globalIdx = 1;
  for (const [nbr, regs] of nbrEntries) {
    const nbrKg  = regs.reduce((s, r) => s + (r.weightKg || 0), 0);
    const nbrPct = totalKg > 0 ? Math.round((nbrKg / totalKg) * 100) : 0;
    const sorted = [...regs].sort((a, b) => ORDER.indexOf(a.status) - ORDER.indexOf(b.status));
    const rows = sorted.map((r) => {
      const row = `<tr><td>${globalIdx++}</td><td>${r.fullName || "—"}</td><td>${r.phone || "—"}${r.phoneUnverified ? " ⚠️" : ""}</td><td>${r.cargoDesc || "—"}</td><td class="num">${r.weightKg || 0}</td><td class="status status-${r.status}">${PRINT_STATUS[r.status] || r.status}</td></tr>`;
      return row;
    }).join("");
    nbrDetailBlocks += `
<div class="nbr-block">
  <div class="nbr-header">
    🏘 ${nbr}
    <span class="nbr-meta">${regs.length} ሰው &nbsp;|&nbsp; ${nbrKg} ኪሎ &nbsp;|&nbsp; ${nbrPct}%</span>
  </div>
  <table>
    <thead><tr><th>#</th><th>ሙሉ ስም</th><th>ስልክ ቁጥር</th><th>የጭነት ዓይነት</th><th class="num">ኪሎ</th><th class="status">ሁኔታ</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
</div>`;
  }

  const css = `@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:'Noto Sans Ethiopic','Nyala',Arial,sans-serif;color:#1a1a1a;margin:0;padding:18px;font-size:12px}.letterhead{display:flex;justify-content:space-between;border-bottom:3px solid #1a3c6e;padding-bottom:10px;margin-bottom:12px}.letterhead h1{font-size:17px;margin:0 0 3px;color:#1a3c6e}.letterhead .meta{text-align:right;font-size:11px}.route-banner{background:#1a3c6e;color:#fff;padding:7px 14px;border-radius:4px;font-size:14px;font-weight:bold;margin-bottom:12px}.summary{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.box{border:1px solid #ccc;border-radius:5px;padding:6px 12px;text-align:center;background:#f7f8fa;min-width:90px}.box .v{font-size:17px;font-weight:bold;color:#1a3c6e}.box .l{font-size:10px;color:#666;margin-top:2px}h3{color:#1a3c6e;font-size:13px;margin:14px 0 6px;border-bottom:1px solid #cdd6e0;padding-bottom:4px}table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:14px}th{background:#1a3c6e;color:#fff;padding:6px 5px;text-align:left}td{padding:5px;border-bottom:1px solid #ddd}tr:nth-child(even){background:#f5f6f8}.num{text-align:center}.status{text-align:center;font-weight:bold;font-size:10px}.status-approved{color:#1a7d3b}.status-sent{color:#1565c0}.status-reviewing{color:#b8860b}.status-pending{color:#888}.nbr-block{margin-bottom:18px;page-break-inside:avoid}.nbr-header{background:#e8edf5;border-left:4px solid #1a3c6e;padding:5px 10px;font-weight:bold;font-size:12px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center}.nbr-meta{font-size:11px;color:#1a3c6e;font-weight:normal}.footer{margin-top:28px;display:flex;justify-content:space-between;font-size:11px}.sign-box{width:45%}.sign-line{border-top:1px solid #333;margin-top:34px;padding-top:4px;text-align:center}.stamp-note{margin-top:20px;font-size:10px;color:#777;text-align:center;border-top:1px dashed #ccc;padding-top:8px}.checkpoint-box{border:2px solid #1a3c6e;border-radius:6px;padding:10px 14px;margin-bottom:14px;background:#f0f4fa}.checkpoint-box h2{margin:0 0 6px;font-size:13px;color:#1a3c6e}.checkpoint-row{display:flex;gap:24px;flex-wrap:wrap}.checkpoint-item{font-size:12px}.checkpoint-item span{font-weight:bold;font-size:15px;color:#1a3c6e}#printBtn{margin:12px 0;padding:9px 26px;font-size:13px;background:#1a3c6e;color:#fff;border:none;border-radius:6px;cursor:pointer}@media print{#printBtn{display:none}body{padding:0}}`;
...
<div class="route-banner">${ro.emoji} ${ro.label}${filterNbr ? ` &nbsp;›&nbsp; 🏘 ${filterNbr}` : ""}</div>

<!-- ኬላ ፈታሽ / ተቆጣጣሪ ማጠቃለያ -->
<div class="checkpoint-box">
  <h2>📋 ለኬላ ፈታሽ / ጭነት ተቆጣጣሪ — ማጠቃለያ</h2>
  <div class="checkpoint-row">
    <div class="checkpoint-item">ብዛት<br><span>${list.length}</span></div>
    <div class="checkpoint-item">ጠቅላላ ኪሎ<br><span>${totalKg}</span></div>
    <div class="checkpoint-item">ፈቃድ ያላቸው<br><span>${cnt.approved + cnt.sent}</span></div>
    <div class="checkpoint-item">ሰፈሮች<br><span>${nbrEntries.length}</span></div>
    <div class="checkpoint-item">ቀን<br><span style="font-size:12px">${dateStr}</span></div>
  </div>
</div>
...
<div class="summary">
  ...
  <div class="box"><div class="v">${totalReg.toLocaleString("en")}</div><div class="l">ምዝ. ክፍያ (ብር)</div></div>
  <div class="box"><div class="v">${totalShip.toLocaleString("en")}</div><div class="l">ጭነት ክፍያ (ብር)</div></div>
</div>

<!-- በሰፈር ድርሻ -->
<h3>🏘 በሰፈር ማጠቃለያ</h3>
<table>
  <thead><tr><th>ሰፈር</th><th class="num">ብዛት</th><th class="num">ኪሎ</th><th class="num">ድርሻ %</th></tr></thead>
  <tbody>${nbrSummaryRows}</tbody>
</table>
...
function buildGBManifestHTML(prod, list, filterNbr = null) {
  const ul       = unitLabel(prod);
  const totalKg  = list.reduce((s, r) => s + (r.weightKg || 0), 0);
  ...
  const nbrSummaryRows = nbrEntries.map(([nbr, regs]) => {
    ...
    const pct = totalKg > 0 ? Math.round((kg / totalKg) * 100) : 0;
    return `<tr><td>${nbr}</td><td class="num">${cnt}</td><td class="num">${kg} ${ul}</td><td class="num">${pct}%</td></tr>`;
  }).join("") +
  `<tr style="font-weight:bold;background:#dde4ef"><td>ጠቅላላ</td><td class="num">${list.length}</td><td class="num">${totalKg} ${ul}</td><td class="num">100%</td></tr>`;
  ...
  for (const [nbr, regs] of nbrEntries) {
    ...
    const rows = regs.map((r) =>
      `<tr><td>${globalIdx++}</td><td>${r.fullName || "—"}</td><td>${r.phone || "—"}</td><td>${r.neighborhood || "—"}</td><td class="num">${r.weightKg || 0} ${ul}</td></tr>`
    ).join("");
    nbrDetailBlocks += `
<div class="nbr-block">
  <div class="nbr-header">
    🏘 ${nbr}
    <span class="nbr-meta">${regs.length} ሰው &nbsp;|&nbsp; ${nbrKg} ${ul} &nbsp;|&nbsp; ${nbrPct}%</span>
  </div>
  <table>
    <thead><tr><th>#</th><th>ሙሉ ስም</th><th>ስልክ ቁጥር</th><th>ሰፈር</th><th class="num">${ul}</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
</div>`;
  }

  const css = `@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:'Noto Sans Ethiopic','Nyala',Arial,sans-serif;color:#1a1a1a;margin:0;padding:18px;font-size:12px}.letterhead{display:flex;justify-content:space-between;border-bottom:3px solid #1a3c6e;padding-bottom:10px;margin-bottom:12px}.letterhead h1{font-size:17px;margin:0 0 3px;color:#1a3c6e}.letterhead .meta{text-align:right;font-size:11px}.route-banner{background:#1a3c6e;color:#fff;padding:7px 14px;border-radius:4px;font-size:14px;font-weight:bold;margin-bottom:12px}.summary{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.box{border:1px solid #ccc;border-radius:5px;padding:6px 12px;text-align:center;background:#f7f8fa;min-width:90px}.box .v{font-size:17px;font-weight:bold;color:#1a3c6e}.box .l{font-size:10px;color:#666;margin-top:2px}h3{color:#1a3c6e;font-size:13px;margin:14px 0 6px;border-bottom:1px solid #cdd6e0;padding-bottom:4px}table{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:12px}th{background:#1a3c6e;color:#fff;padding:6px 5px;text-align:left}td{padding:5px;border-bottom:1px solid #ddd}tr:nth-child(even){background:#f5f6f8}.num{text-align:center}.nbr-block{margin-bottom:16px;page-break-inside:avoid}.nbr-header{background:#e8edf5;border-left:4px solid #1a3c6e;padding:5px 10px;font-weight:bold;font-size:12px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center}.nbr-meta{font-size:11px;color:#1a3c6e;font-weight:normal}.footer{margin-top:28px;display:flex;justify-content:space-between;font-size:11px}.sign-box{width:45%}.sign-line{border-top:1px solid #333;margin-top:34px;padding-top:4px;text-align:center}#printBtn{margin:12px 0;padding:9px 26px;font-size:13px;background:#1a3c6e;color:#fff;border:none;border-radius:6px;cursor:pointer}@media print{#printBtn{display:none}body{padding:0}}`;

  return `<!DOCTYPE html><html lang="am"><head><meta charset="UTF-8"><title>${prod.label} — የቡድን ግዥ ዝርዝር</title><style>${css}</style></head><body>
<button id="printBtn" onclick="window.print()">🖨 ፕሪንት ያድርጉ</button>
...
<div class="route-banner">${prod.emoji} ${prod.label}${filterNbr ? ` &nbsp;›&nbsp; 🏘 ${filterNbr}` : ""}</div>
...
<div class="summary">
  ...
  <div class="box"><div class="v">${nbrEntries.length}</div><div class="l">ሰፈሮች</div></div>
</div>

<h3>🏘 በሰፈር ማጠቃለያ</h3>
<table>
  <thead><tr><th>ሰፈር</th><th class="num">ብዛት</th><th class="num">${ul}</th><th class="num">ድርሻ %</th></tr></thead>
  <tbody>${nbrSummaryRows}</tbody>
</table>
...
bot.action("gb_status", async (ctx) => {
  ...
  for (const prod of GB_PRODUCTS) {
    const ul   = unitLabel(prod);
    const list = await GBReg.find({ productId: prod.id }).lean();
...
bot.action(/^prntgb_(.+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `${prod.emoji} *${prod.label}* — ሰፈር ምረጥ:`,
    {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("📋 ሁሉም ሰፈሮች", `prntgb_nbr_${prodId}__ALL__`)],
...
bot.action(/^prntgb_nbr_(.+?)__(.+)$/, async (ctx) => {
  ...
  await handlePrintGB(ctx, prodId, nbr === "__ALL__" ? null : nbr);
});
bot.action(/^prnt_(.+)$/, async (ctx) => {
  if (!isAdmin(ctx)) { await ctx.answerCbQuery("ፈቃድ የለዎትም").catch(() => {}); return; }
  await ctx.answerCbQuery().catch(() => {});
  const routeId = ctx.match[1];
  /* prntgb_ ን ሰርዝ (ይህ regex ቀደም ሲል ሊወሰን ስለሚችል) */
  if (routeId.startsWith("gb_pick") || routeId.startsWith("dir_")) return;
  const ro = byRoute(routeId);
  if (!ro) return;
  /* ሰፈሮቹን ዘርዝር */
  const regs = await Reg.find({ routeId, status: { $ne: "rejected" } }).lean();
  const nbrs = [...new Set(regs.map((r) => (r.neighborhood || "ሌላ").trim()))].sort((a, b) => a.localeCompare(b));
  if (!nbrs.length) return ctx.reply(`${ro.emoji} ${ro.label} — ምዝገባ የለም`);
  await ctx.reply(
    `${ro.emoji} *${ro.label}* — ሰፈር ምረጥ:`,
    {
      parse_mode: "Markdown",
      ...Markup.inlineKeyboard([
        [Markup.button.callback("📋 ሁሉም ሰፈሮች", `prnt_nbr_${routeId}__ALL__`)],
        ...nbrs.map((n) => [Markup.button.callback(`🏘 ${n}`, `prnt_nbr_${routeId}__${n}`)]),
        [Markup.button.callback("🔙 ተመለስ", "print_pick")],
      ]),
    }
  );

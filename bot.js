...
/* ሰራተኞች (Staff) IDs — የቤት ምዝገባ ማየት/ማጽደቅ ብቻ ይፈቀዳቸዋል */
const STAFF_IDS = (process.env.STAFF_IDS || "")
  .split(",")
  .map((s) => Number(s.trim()))
  .filter(Boolean);

/* ቻናል — ደንበኛ ራሱ ሲመዘገብ ራሱ-ሰር (automatically) የሚለጠፍበት (ለምሳሌ: @mychannel ወይም -100123456789) */
...
/* ─── 5. HELPERS ────────────────────────────────────────── */
/* ── Dynamic Admins — DB ላይ ተከማችተው በ /addadmin ይታከላሉ ────────
   ADMIN_IDS (ከ env) ሁልጊዜ Super Admin ናቸው — እነሱ ብቻ admin መጨመር/ማውጣት ይችላሉ */
let EXTRA_ADMIN_IDS = [];
async function loadExtraAdmins() {
  try { EXTRA_ADMIN_IDS = await getSetting("extra_admins", []); }
  catch { EXTRA_ADMIN_IDS = []; }
}
const isSuperAdmin = (ctx) => ADMIN_IDS.includes(ctx.from?.id);
const isAdmin      = (ctx) => ADMIN_IDS.includes(ctx.from?.id) || EXTRA_ADMIN_IDS.includes(ctx.from?.id);
/* Staff — Admin ወይም ሰራተኛ ከሆነ true */
const isStaff     = (ctx) => isAdmin(ctx) || STAFF_IDS.includes(ctx.from?.id);
const isStaffOnly = (ctx) => STAFF_IDS.includes(ctx.from?.id) && !isAdmin(ctx);

/* ─── 6. KEYBOARDS ──────────────────────────────────────── */
function mainKb(userId) {
  const isAdminUser = ADMIN_IDS.includes(userId) || EXTRA_ADMIN_IDS.includes(userId);
...
/* ─── Dynamic Admin Management (Super Admin ብቻ) ─────────────── */
function extractUserId(ctx, txt) {
...
bot.command("addadmin", async (ctx) => {
  if (!isSuperAdmin(ctx)) return ctx.reply("ፈቃድ የለዎትም — Super Admin ብቻ ይህን ማድረግ ይችላል።");
  const arg = ctx.message.text.split(" ").slice(1).join(" ").trim();
  const uid = extractUserId(ctx, arg);
  if (!uid) return ctx.reply("👤 አድርግ የሚፈልጉትን ሰው User ID ያስገቡ:\n`/addadmin 123456789`\n\n_ወይም የሰውየውን መልእክት reply አድርገው `/addadmin` ይላኩ_", { parse_mode: "Markdown" });
  if (ADMIN_IDS.includes(uid) || EXTRA_ADMIN_IDS.includes(uid))
    return ctx.reply("✅ ይህ ሰው አስቀድሞ admin ነው።");
  EXTRA_ADMIN_IDS.push(uid);
  await setSetting("extra_admins", EXTRA_ADMIN_IDS);
  await ctx.reply(`✅ *${uid}* ሙሉ admin ሆኗል!`, { parse_mode: "Markdown" });
  bot.telegram.sendMessage(uid, "🎉 እንኳን ደስ አለዎት! የ Admin ፈቃድ ተሰጥቶዎታል — /start ብለው ይጀምሩ።").catch(() => {});
});
...
bot.command("removeadmin", async (ctx) => {
  ...
  const arg = ctx.message.text.split(" ").slice(1).join(" ").trim();
  const uid = extractUserId(ctx, arg);
  if (!uid) return ctx.reply("👤 ማውጣት የሚፈልጉትን ሰው User ID ያስገቡ:\n`/removeadmin 123456789`", { parse_mode: "Markdown" });
  if (ADMIN_IDS.includes(uid))
    return ctx.reply("⛔ ይህ ሰው Super Admin ነው (ከ env የተቀመጠ) — ከቦት ኮድ/ENV ውጪ ማውጣት አይቻልም።");
  if (!EXTRA_ADMIN_IDS.includes(uid))
    return ctx.reply("❌ ይህ ሰው admin አይደለም።");
  EXTRA_ADMIN_IDS = EXTRA_ADMIN_IDS.filter((id) => id !== uid);
  await setSetting("extra_admins", EXTRA_ADMIN_IDS);
  await ctx.reply(`✅ *${uid}* ከ admin ወጥቷል።`, { parse_mode: "Markdown" });
  bot.telegram.sendMessage(uid, "ℹ️ የ Admin ፈቃድዎ ተነስቷል።").catch(() => {});
});

bot.command("admins", async (ctx) => {
  if (!isAdmin(ctx)) return ctx.reply("ፈቃድ የለዎትም");
  const superLines = ADMIN_IDS.map((id) => `👑 \`${id}\` (Super Admin)`).join("\n") || "—";
  const extraLines = EXTRA_ADMIN_IDS.map((id) => `🔧 \`${id}\``).join("\n") || "—";
  await ctx.reply(
    `*የ Admin ዝርዝር*\n━━━━━━━━━━━━━━━━\n\n👑 *Super Admins:*\n${superLines}\n\n🔧 *የተጨመሩ Admins:*\n${extraLines}`,
    { parse_mode: "Markdown" },
  );
...
/* ደንበኛ ራሱ ሲመዘገብ ራሱ-ሰር ወደ ቻናል ይለጥፋል (ADMIN ማጽደቅ ሳያስፈልግ) */
async function postListingToChannel(listing) {
  if (!CHANNEL_ID) return;
  const typeLabel = houseTypeLabel(listing.dealType);
  const detailsLine = houseDetailsLine(listing);
  const isSeeking = HOUSE_SEEKING_TYPES.includes(listing.dealType);
  const locLabel   = isSeeking ? "የሚፈልጉት አካባቢ" : "📍";
  const priceLabel = isSeeking ? "በጀት" : "💰";
  const caption =
    `${typeLabel}\n${locLabel}: ${listing.location}\n` +
    (detailsLine ? `${detailsLine}\n` : "") +
    `${priceLabel}: ${listing.price}\n📝 ${listing.description || "—"}\n\n📞 ${SUPPORT_PHONE}`;
  const firstMedia = listing.media?.[0];
  try {
    if (firstMedia?.type === "photo") {
      await bot.telegram.sendPhoto(CHANNEL_ID, firstMedia.fileId, { caption, parse_mode: "Markdown" });
    } else if (firstMedia?.type === "video") {
      await bot.telegram.sendVideo(CHANNEL_ID, firstMedia.fileId, { caption, parse_mode: "Markdown" });
    } else {
      await bot.telegram.sendMessage(CHANNEL_ID, caption, { parse_mode: "Markdown" });
    }
  } catch (e) {
    console.error("postListingToChannel:", e.message);
    notifyAdmins(`⚠️ ወደ ቻናል መለጠፍ አልተሳካም: ${e.message}`);
  }
}
...
async function finalizeHouseListing(ctx) {
  ...
  /* ደንበኛ ራሱ ሲመዘግብ ወዲያውኑ "approved" ተደርጎ ወደ ቻናል ራሱ-ሰር ይለጠፋል */
...
/* ማስታወሻ: ምዝገባዎች ራሱ-ሰር ወደ ቻናል ስለሚለጠፉ "✅ አፅድቅ" አያስፈልግም —
   Staff/Admin ስህተት ወይም አታላይ ምዝገባ ካገኙ "❌ ውድቅ" ወይም "🗑 ሰርዝ" ብቻ ይጠቀማሉ። */
...
bot.action(/^house_rej_([a-f\d]{24})$/i, async (ctx) => {
  ...
  await ctx.editMessageReplyMarkup({ inline_keyboard: [] }).catch(() => {});
  await ctx.reply("❌ ውድቅ ተደርጓል (ቻናል ላይ የተለጠፈ ልጥፍ በእጅ መሰረዝ ያስፈልጋል)");
});
...
async function main() {
  ...
  console.log("Extra admins loaded:", EXTRA_ADMIN_IDS.length);
